...
assert.match(index, /class="game-info-action-hint" id="modal-action-hint"/);
assert.match(index, /abra uma transmissão para outras pessoas assistirem/);
assert.match(index, /@media\(max-width:520px\)\{ \.game-info-actions \{ grid-template-columns:1fr; \} \}/);
assert.match(index, /neo_multiplayer_game_after_auth/);
assert.match(index, /startSpecificMultiplayerGame\(gameObj, playUrl\)/);
assert.match(index, /source:'game_details'/);
