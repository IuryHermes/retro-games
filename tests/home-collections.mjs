...
for (const marker of ['mortal-kombat', 'mega-man', 'showAll.onclick', 'panel.scrollIntoView']) {
  if (!controller.includes(marker)) throw new Error(`controlador de coletâneas incompleto: ${marker}`);
}
for (const marker of ['scroll-behavior:smooth', '-webkit-overflow-scrolling:touch', 'v=20260901-10']) {
  if (!(controller + html).includes(marker)) throw new Error(`rolagem móvel nativa ausente: ${marker}`);
}
if (controller.includes("addEventListener('touchmove'") || controller.includes('onpointerdown')) throw new Error('coletâneas ainda usam gesto manual diferente dos emuladores');
...
console.log('coletâneas da Home validadas: rolagem nativa igual aos emuladores e abertura sem navegação');
