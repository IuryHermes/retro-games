...
assert.match(player, /ps1-mobile-audio\.js\?v=2/);
assert.match(player, /mobilePs1AudioProfile[\s\S]*"vsync": true[\s\S]*"audio_latency": "256"/);
assert.match(player, /"pcsx_rearmed_duping_enable": "enabled"/);
assert.match(player, /"pcsx_rearmed_async_cd": "async"/);
assert.match(profile, /audio_latency = 256/);
...
console.log('PS1 mobile audio profile: 10 checks passed');
