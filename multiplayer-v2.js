(function () {
        ...
        const draw = () => {
            ...
            try {
                ...
                requestAnimationFrame(() => {
                    const visual = target.querySelector('img,canvas');
                    if (!visual) target.textContent = 'QR indisponível. Use COPIAR CONVITE.';
                });
            } catch (error) {
                target.textContent = 'QR indisponível. Use COPIAR CONVITE.';
                console.error('Neo multiplayer QR:', error);
            }
        };
    ...
    function maybeAskSessionFeedback(session) {
        if (!session || session.durationSeconds < 30) return;
        const storageKey = 'neo_multiplayer_feedback_last_at';
        const cooldown = 14 * 24 * 60 * 60 * 1000;
        const lastAsked = Number(localStorage.getItem(storageKey) || 0);
        if (Date.now() - lastAsked < cooldown || Math.random() > 0.35) return;
        localStorage.setItem(storageKey, String(Date.now()));
        const survey = document.createElement('aside');
        survey.id = 'neo-multiplayer-feedback';
        survey.setAttribute('role', 'dialog');
        survey.setAttribute('aria-modal', 'true');
        survey.setAttribute('aria-labelledby', 'neo-multiplayer-feedback-title');
        survey.innerHTML = `<div><button class="neo-feedback-dismiss" type="button" aria-label="Fechar">×</button><strong id="neo-multiplayer-feedback-title">A PARTIDA ONLINE FUNCIONOU BEM?</strong><p>Uma resposta ajuda a melhorar o multiplayer neste aparelho.</p><div class="neo-feedback-actions"><button type="button" data-answer="worked">SIM, FUNCIONOU</button><button type="button" data-answer="problem">TEVE PROBLEMA</button><button type="button" data-answer="could_not_play">NÃO CONSEGUI JOGAR</button></div><small>Não pedimos dados pessoais.</small></div>`;
        const close = () => survey.remove();
        survey.querySelector('.neo-feedback-dismiss').onclick = close;
        survey.addEventListener('click', event => { if (event.target === survey) close(); });
        survey.querySelectorAll('[data-answer]').forEach(button => {
            button.onclick = () => {
                window.neoTrack?.('multiplayer_session_feedback', {
                    answer: button.dataset.answer,
                    game_system: session.system || 'unknown',
                    duration_group: session.durationSeconds < 120 ? 'under_2m' : session.durationSeconds < 600 ? '2_to_10m' : 'over_10m',
                    device_category: matchMedia('(pointer:coarse)').matches ? 'touch' : 'desktop'
                });
                survey.querySelector('div').innerHTML = '<strong>OBRIGADO!</strong><p>Sua resposta foi registrada e vai ajudar nas próximas melhorias.</p>';
    ...
    function closeRoom() {
        const finishedSession = room ? { system, durationSeconds:Math.max(0, Math.round((Date.now() - playSessionStartedAt) / 1000)) } : null;
        ...
        setTimeout(() => maybeAskSessionFeedback(finishedSession), 500);
    }
    ...
    function createPanel() {
        ...
        style.textContent += '#neo-multi-qr-code canvas,#neo-multi-qr-code img{display:block!important;width:156px!important;height:156px!important;max-width:100%}#neo-multiplayer-feedback{position:fixed;inset:0;z-index:10000002;display:grid;place-items:center;padding:18px;background:rgba(0,0,0,.78);color:#eaffef;font:13px monospace}#neo-multiplayer-feedback>div{position:relative;width:min(430px,calc(100vw - 36px));box-sizing:border-box;padding:22px;border:1px solid #35ef70;border-radius:8px;background:#07120b;box-shadow:0 0 28px rgba(53,239,112,.25);text-align:center}#neo-multiplayer-feedback strong{display:block;color:#70ff9b;font-size:16px}#neo-multiplayer-feedback p{line-height:1.5}#neo-multiplayer-feedback small{display:block;margin-top:12px;color:#91a898}.neo-feedback-dismiss{position:absolute;right:8px;top:6px;border:0!important;background:transparent!important;color:#b9c9bd!important;font-size:24px;cursor:pointer}.neo-feedback-actions{display:grid;gap:8px}.neo-feedback-actions button{padding:11px;border:1px solid #35ef70;background:#102b18;color:#fff;font:700 12px monospace;cursor:pointer}.neo-feedback-actions button:focus-visible{outline:2px solid #fff;outline-offset:2px}';
